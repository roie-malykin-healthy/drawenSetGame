...
struct Grid {
    ...
    subscript(row: Int, column: Int) -> CGRect? {
      self[row * dimensions.columnCount + column]
    }
    subscript(index: Int) -> CGRect? {
      index < cellFrames.count ? cellFrames[index] : nil
    }
    ...
    var cellSize: CGSize {
      get { cellFrames.first?.size ?? CGSize.zero }
    ...
    private mutating func updateCellFrames(to cellSize: CGSize) {
      cellFrames.removeAll()
      let boundingSize = CGSize( width: CGFloat(dimensions.columnCount) * cellSize.width, height: CGFloat(dimensions.rowCount) * cellSize.height
      )
      let offset = (
